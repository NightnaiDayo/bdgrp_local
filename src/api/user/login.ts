...
import { UserLogin, UserRegistration } from "@proto";
...
router.post('/', async (req, res) => {
    ...
    const decoded = UserLogin.decode(buffer);

    let user = db.Users[process.env.SERVER].find((u: any) => BigInt(u.bid) == BigInt(decoded.bid));
    ...
    const data = {
        ...
        unknownString: user.unknownString,
