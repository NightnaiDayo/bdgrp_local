...
import { UserDecoSituationUseProfileSettingRequest, UserDecoEquipmentGetResponse } from "@proto"
...
router.put('/', async(req, res) => {
    const decoded = UserDecoSituationUseProfileSettingRequest.decode(decrypt(req.body))
    // @ts-ignore
    const userid = req.params.userid
    const user = db.Users[process.env.SERVER].find((u: any) => u.userId == userid);

    user.useProfileSettingSituation = decoded.useProfileSettingSituation
    saveDb()
